...
//    auth_result = gen_auth_res_xor(rand, autn_enb, res, &res_len, ak_xor_sqn);
  ...
  if (auth_result == AUTH_OK) {
    ...
    srsran::console("5G RNAKA Authentication Successfully \n");
    // Generate RES STAR
    security_generate_res_star(ck, ik, serving_network_name, rand, res, res_len, res_star);
    logger.debug(res_star, 16, "RES STAR");
    ...
    security_generate_k_amf(k_seaf, imsi_str.c_str(), abba, abba_len, k_amf);
    logger.debug(k_amf, 32, "K AMF");
    std::string kaust_str;
    std::string kseaf_str;
    std::string kamf_str;
       for (size_t i = 0; i < 32; i++)
      {
        char byte_str[4];  // 2 digits for hex + 1 space or null terminator
        char byte2_str[4];  // 2 digits for hex + 1 space or null terminator
        char byte3_str[4];  // 2 digits for hex + 1 space or null terminator
        std::snprintf(byte_str, sizeof(byte_str), "%02X ", k_ausf[i]);
        std::snprintf(byte2_str, sizeof(byte2_str), "%02X ", k_seaf[i]);
        std::snprintf(byte3_str, sizeof(byte3_str), "%02X ", k_amf[i]);
        kaust_str += byte_str;  // 将每个字节追加到结果字符串中
        kseaf_str += byte2_str;  // 将每个字节追加到结果字符串中
        kamf_str += byte3_str;  // 将每个字节追加到结果字符串中
      }
      srsran::console("KAUSF: %s\n", kaust_str.c_str());
      srsran::console("KSEAF*: %s\n", kseaf_str.c_str());
      srsran::console("KAMF*: %s\n", kamf_str.c_str());
  }
  return auth_result;
}
...
auth_result_t usim::gen_auth_res_milenage_new(uint8_t* rand, uint8_t* autn_enb, uint8_t* snmac, uint8_t* res, int* res_len, const char* serving_network_name)
{
  ...
  *res_len = 8;
  ...
  // 调用f1函数来计算MAC（使用新的函数）
  // Generate MAC
  security_milenage_f1_new(k, opc, rand, mac);//指的是xHNMAC  BY HUAHUA

  // 调用f1star函数来计算AK
  security_milenage_f1_star_new(k, opc, rand, ak_new);//指的是AK BY HUAHUA
  //下面就是AK XOR AUTN --> HNMAC
  for(i = 0; i < 8; i++)
  {
     hnmac[i] = ak_new[i] ^ autn_enb[i];
  }
  
  //Compare HNMAC with xHNMAC
  for(i = 0;i < 8;i++)
  {
    if(hnmac[i] != mac[i])
    {
  ...
  std::string xhnmac_str;
  std::string hnmac_str;
  if(result != AUTH_FAILED)
  {
     for (size_t i = 0; i < 8; ++i)
    {
      char byte_str[4];  // 2 digits for hex + 1 space or null terminator
      std::snprintf(byte_str, sizeof(byte_str), "%02X ", hnmac[i]);
      xhnmac_str += byte_str;  // 将每个字节追加到结果字符串中
      hnmac_str += byte_str;
    }
    srsran::console("XHNMAC: %s\n", xhnmac_str.c_str());
    srsran::console("HNMAC: %s\n",hnmac_str.c_str());
    srsran::console("XHNMAC = HNMAC\n");
  }
  else
  {
        srsran::console("SNMAC != XSNMAC");
  }
  // 这里的逻辑和协议本身有点不对，但是也是可以的。按照协议来说直接比较两个MAC就行，他这里是根据上面计算得到的重新构建了一个AUTN，然后比较的是两个AUTN的值
  // AUTN变为HNMAC xor AK
  // Construct AUTN
...
//  for (size_t i = 0; i < snmac.size(); ++i) {
//    char byte_str[4];  // 2 digits for hex + 1 space or null terminator
//    std::snprintf(byte_str, sizeof(byte_str), "%02X ", snmac[i]);
//    snmac_str += byte_str;  // 将每个字节追加到结果字符串中
//  }
  std::string snmac_str;
  std::string xsnmac_str;
  if(result != AUTH_FAILED)
  {
     for (size_t i = 0; i < 8; ++i)
    {
